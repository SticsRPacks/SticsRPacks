...
```{r, eval=params$eval_auto_vignette}
## TODO: include this function in SticsRFiles
get_dev_stages <- function(sim, devVars=c("iplts", "igers", "ilevs", "iamfs",
  ...
  for(usm_name in usm){
    ...
    out_usm <- data.frame(var=devVars,
                          day=NA,
                          date="")
...
```{r}
newSowingDensities <- c(seq(5, 50, by=5),
                        seq(50, 1000, by=25),
...
for(newSowDens in newSowingDensities){
  ...
  if(file.exists(outF))
    file.remove(outF)
  set_param_xml(file.path(workspace, usm_name, usm_details$ftec[1]),
...
gen_usms_xml(file.path(workspace, "usms.xml"),
             param_df)

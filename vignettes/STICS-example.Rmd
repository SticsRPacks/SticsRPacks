...
```{r, eval=params$eval_vignette == "run"}
extract_workspace(
  from_workspace = file.path(javastics, "example"),
...
usm_details <-
  get_param_xml(file.path(javastics, "example", "usms.xml"),
    select = "usm", select_value = usm_name
  )[[1]]
```
...
get_usm_years <- function(workspace, usmSelVal, usmsFile = "usms.xml") {
  ...
  fclim1 <- get_param_xml(
    file = file.path(workspace, usmsFile),
    param = "fclim1", select = "usm",
    select_value = usmSelVal
  )[[1]]$fclim1
...
```{r, eval=params$eval_auto_vignette}
## TODO: include this function in SticsRFiles
get_dev_stages <- function(sim, devVars = c(
                             "iplts", "igers", "ilevs", "iamfs",
  ...
  for (usm_name in usm) {
    ...
    for (i in 1:nrow(out_usm)) {
      devVar <- devVars[i]
      if (!devVar %in% colnames(dat)) {
        msg <- paste0("'", devVar, "' not found")
        warning(msg, immediate. = TRUE)
        next
      }
      tmp <- table(dat[[devVar]])
      if (length(tmp) == 1) { # ex. iplts
        ...
        stop(paste0(devVar, " has more than three values"))
      }
...
(devStages <- get_dev_stages(sim, devVars))
```
...
for (out_var in c("masec_n", "hauteur", "lai_n")) {
  p <- plot(sim, var = out_var, obs = obs)[[usm_name]] +
    geom_vline(data = devStages[[usm_name]], aes(xintercept = Date, color = var)) +
    theme_bw()
  print(p)
}
```
...
We will explore the range of possible sowing densities, and make an USM for each.
...
```{r}
newSowingDensities <- c(
  seq(5, 50, by = 5),
  seq(50, 1000, by = 25),
  seq(1000, 2000, by = 100)
)
newSowingDensities <- unique(newSowingDensities)

get_param_xml(file.path(workspace, usm_details$ftec[1]), "densitesem")
if (FALSE) {
  prefix <- tools::file_path_sans_ext(usm_details$ftec[1])
  prefix <- gsub("_tec", "", prefix)
  for (newSowDens in newSowingDensities) {
    outF <- file.path(workspace, usm_name, paste0(prefix, "_densitesem", newSowDens, "_tec.xml")) # compulsory to end by _tec.xml
    if (file.exists(outF)) {
      file.remove(outF)
    }
    set_param_xml(
      file.path(workspace, usm_name, usm_details$ftec[1]),
      "densitesem", newSowDens,
      outF
    )
  }

  (tmp <- get_param_xml(file.path(workspace, "usms.xml"),
    select = "usm", select_value = usm_name
  ))
  pars <- list()
  for (newSowDens in newSowingDensities) {
    new_usm <- paste0(usm_name, "_densitesem", newSowDens)
    param_df <- data.frame(usm_name = new_usm)
    for (i in seq_along(tmp[[1]])) {
      if (length(tmp[[1]][[i]]) == 1) {
        param_df[[names(tmp[[1]])[i]]] <- tmp[[1]][i][[1]][1]
      } else if (length(tmp[[1]][[i]]) == 2) {
        param_df[[paste0(names(tmp[[1]])[i], "_1")]] <- tmp[[1]][[i]][1]
        param_df[[paste0(names(tmp[[1]])[i], "_2")]] <- tmp[[1]][[i]][2]
      } else {
        stop(paste0("error: more than 2 values at i=", i))
      }
    }
    param_df$ftec_1 <- paste0(prefix, "_densitesem", newSowDens, "_tec.xml") # compulsory to end by _tec.xml
    pars[[as.character(newSowDens)]] <- param_df
  }
  param_df <- do.call(rbind, pars)
  gen_usms_xml(
    file.path(workspace, "usms.xml"),
    param_df
  )
  get_usms_list(file.path(workspace, "usms.xml"))
}
```
...
```{r, echo=FALSE}
var_name <- ""
if (params$eval_vignette == "run") {
  precomputed[[var_name]] <- get(var_name)
} else if (params$eval_vignette == "load") {
  assign(var_name, precomputed[[var_name]])
}
```

```{r save_list_of_precomputed_variables, eval=params$eval_vignette == "run", echo=FALSE}
output_dir_path <- file.path(getwd(), "..", "inst", "extdata")
stopifnot(dir.exists(output_dir_path))
output_file_path <- file.path(output_dir_path, params$precomputedFile)
if (file.exists(output_file_path)) {

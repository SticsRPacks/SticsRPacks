...
## Running simulations from R {#run-sim}
...
> Get simulations outputs of USMs from the workspace directory used in the previous exercise for running the model
...
* Get and keep in environment simulations outputs using the variable `lai(n)` for the USMs named   
`wheat` and `maize`
...
* Plot simulations dynamics of the variable `lai(n)` and `masec` for `wheat`
  * Independently

<!-- for the USMs named `wheat` and `maize` -->
...
Evaluation of simulation results is performed using observation data for plotting dynamics, scatter graphs and calculating several statistics. Observed data are to be loaded by using the `get_obs` function

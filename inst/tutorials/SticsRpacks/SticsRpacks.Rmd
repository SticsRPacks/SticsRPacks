...
* Use the values of the variables `lai_n` and `masec_n` simulated for USM `wheat` already stored in the `results` object and plot their dynamics

<!-- calling prepare-get_sim in case ... -->
```{r prepare-plot_sim, echo=FALSE, message=FALSE, results='hide'}
file_path <- file.path(tmp_dir,"sim.RData")
...
As for the first exercise in the "Plotting simulations results" section, use the values of the variables `lai_n` and `masec_n` simulated for USM `wheat` stored in the `results2` object and plot their dynamics, but this time add the observation data to the plot using the values stored in the `observations` object. 

```{r prepare-plot_sim_obs, echo=FALSE, message=FALSE, results='hide'}
file_path <- file.path(tmp_dir,"sim_obs.RData")
if (!file.exists(file_path)) {
  results2 <- get_sim(workspace = workspace_xml, usm_name = c("wheat", "maize"), verbose = FALSE)
  observations <- get_obs(workspace = workspace_xml, usm_name = c("wheat", "maize"), verbose = FALSE)
  save(list=c("results2","observations"), file=file_path)
} else {
  #print("loading")
  load(file=file_path)
}
...
```{r plot_simobs1, exercise=TRUE, exercise.setup = "prepare-plot_sim_obs"}
# Insert your R code here!
```
...
# SOLUTION
plot(results2, var = c("lai_n","masec_n"), obs = observations)
```
...
```{r plot_simobs2, exercise=TRUE, exercise.setup = "prepare-plot_sim_obs"}
# Insert your R code here!
```
...
# SOLUTION
plot(results2, var = c("lai_n","masec_n"), obs = observations, type = "scatter")
...
```{r plot_simobs3, exercise=TRUE, exercise.setup = "prepare-plot_sim_obs"}
# Insert your R code here!
```
...
# SOLUTION
plot(results2, var = c("lai_n","masec_n"), obs = observations, type = "scatter", shape_sit = "symbol")
...
if (!file.exists(file.path(tmp_dir,"wrapper_options.RData"))) {
  wrapper_options = stics_wrapper_options(javastics_path = javastics, data_dir = workspace_xml, parallel = TRUE)
  save(list = "wrapper_options", file=file.path(tmp_dir,"wrapper_options.RData"))
} else {
  #print("loading")
  load(file="wrapper_options.RData")
}
```

...
```{r prepare-get_climate, echo=FALSE, message=FALSE, results='hide'}
...
The simulations (and observations) are stored using a `list` of `data.frame`s. Each `data.frame` is used to store the data from a USM, with each row being an observation (*e.g.* the daily `lai`), and each column a variable.
...
```{r prepare-sim_obs, echo=FALSE, message=FALSE, results='hide'}
...
```{r prepare-plot_sim, echo=FALSE, message=FALSE, results='hide'}
...
```{r prepare-plot_sim_obs2, echo=FALSE, message=FALSE, results='hide'}
...
```{r prepare-plot_sim_obs_4, echo=FALSE, message=FALSE, results='hide'}
usms <- c("wheat","DurumWheat", "maize", "sorghum")
...
```{r prepare-wrapper, echo=FALSE, message=FALSE, results='hide'}
# generate missing dirs, if any
dirs <- c("wheat","banana","soybean")
dirs_ex <- dir.exists(file.path(workspace_xml,dirs))
if (!all(dirs_ex)) gen_usms_xml2txt(javastics, workspace_xml, usms_list = dirs[!dirs_ex])
...
```{r prepare-wrapper-plot, echo=FALSE, message=FALSE, results='hide' }
wrapper_options = stics_wrapper_options(javastics_path = javastics, data_dir = workspace_xml, parallel = TRUE)
...
```{r prepare-param-estim, echo=FALSE, message=FALSE, results='hide'}
wrapper_options = stics_wrapper_options(javastics_path = javastics, data_dir = workspace_xml, parallel = TRUE)
...
```{r prepare-param-estim-out, echo=FALSE, message=FALSE, results='hide'}
...
```{r prepare-intercrop-2, echo=FALSE, message=FALSE, results='hide'}
run_javastics(javastics_path=javastics,  workspace_path = workspace_xml, usms_list = c("intercrop_pea_barley"))
...
```{r prepare-intercrop-6, echo=FALSE, message=FALSE, results='hide'}
sim <- get_sim(workspace = workspace_xml, usm_name="intercrop_pea_barley",
               var_list=c("lai_n","masec_n","mafruit"),
               usms_filepath = file.path(workspace_xml,"usms.xml"), javastics_path=javastics,
               verbose = FALSE)

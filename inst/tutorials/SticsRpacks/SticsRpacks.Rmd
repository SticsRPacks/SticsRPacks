...
```{r data_struct2, exercise=TRUE, exercise.setup = "prepare-sim_obs"}
# Insert your R code here!
```
...
```{r data_struct3, exercise=TRUE, exercise.setup = "prepare-sim_obs"}
# Insert your R code here!
```
...
```{r lapply_sim, exercise=TRUE, exercise.setup = "prepare-sim_obs"}
...
```{r data_struct4, exercise=TRUE, exercise.setup = "prepare-sim_obs"}
# Insert your R code here!
```
...
```{r filter_sim, exercise=TRUE, exercise.setup = "prepare-sim_obs"}
sim_all = bind_rows_sim(sim)
...
```{r summarise_sim, exercise=TRUE, exercise.setup = "prepare-sim_obs"}
sim_all%>%
  group_by(situation)%>%
...
```{r list_txt_files, exercise=TRUE}
# Insert your R code here!
```
...
rm(res)
wrapper_options = stics_wrapper_options(javastics_path = javastics_path, data_dir = workspace_path, parallel = TRUE, verbose = FALSE)
obs_filt = filter_obs(get_obs(workspace = workspace_path, usm_name = "wheat", verbose = FALSE), "lai_n", include = TRUE)
...
```{r prepare-intercrop-2}
run_javastics(javastics_path=javastics_path,  workspace_path = workspace_path, usms_list = c("intercrop_pea_barley"), verbose = FALSE)
sim <- get_sim(workspace = workspace_path, usm_name="intercrop_pea_barley",var_list=c("lai_n","masec_n","mafruit"), verbose = FALSE)
```

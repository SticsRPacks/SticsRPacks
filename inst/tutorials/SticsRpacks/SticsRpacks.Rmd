...
Or summarize it by group. For example if we want the maximum lai for each USM (as we did above using `lapply`), we would do as follows:
...
#SOLUTION:
gen_varmod(workspace_path, var_names = c("lai_n", "masec_n", "mafruit"))
```
...
As a reminder, here are the objects we previously defined:
...
It can however be useful if one wants for example to include decision rules between the successive USMs to simulate.
</div>
...
Note however that by default the same values of the parameters will be applied on all USMs. If one wants to estimate the parameters for specific USMs only, or with different values depending on the USMs, one must use the `sit_list` element of the `param_info` argument.
...
* Examine the results and try to answer the quiz below

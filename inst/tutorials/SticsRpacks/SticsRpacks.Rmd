...
javastics <- file.path(work_dir, javastics_dir, fsep ="/")
...
workspace_xml <- file.path(work_dir, workspace_xml, fsep ="/")
...
First display the folder path of the used workspace (contained in  `workspace_xml`) and list its content here after 
...
# use the object name workspace_xml to display its content   
# use the list.files for getting files list of a folder
```
Now, display the USMs list in the workspace `workspace_xml`
...
```{r get_climate_txt2-solution, exercise.setup = "prepare-get_climate"}
#SOLUTION:
...
* Run all the USMs defined in the `example` workspace (path stored in `workspace_xml`). The path to JavaStics is stored in the `javastics` R object.
...
if (!sim_done) {
  ...
  run_javastics(javastics_path = javastics,
                workspace_path = workspace_xml, verbose = FALSE) 
}
```
...
Let's import some simulations:

```{r prepare-sim_obs}
usms <- c("wheat","DurumWheat", "maize", "sorghum")
obs <- get_obs(workspace = workspace_xml, usm_name = usms, verbose = FALSE)
sim <- get_sim(workspace = workspace_xml, usm_name = usms, verbose = FALSE)
sim_all = bind_rows_sim(sim)
```

We now have two objects: `sim` for the simulation results, and `obs` for the observations.

You can get the USMs names using the `names` function on the `sim` and `obs` objects.
Let's do it on the simulation results:
...
```{r data_struct1-solution, exercise.setup = "prepare-sim_obs"}
# SOLUTION
names(sim)
```
...
```{r data_struct2-solution, exercise.setup = "prepare-sim_obs"}
# SOLUTION
sim[[2]]
```
...
```{r data_struct3-solution, exercise.setup = "prepare-sim_obs"}
# SOLUTION
sim$maize
```
...
```{r data_struct4-solution, exercise.setup = "prepare-sim_obs"}
# SOLUTION
bind_rows_sim(sim)
```
...
<!-- calling prepare-get_sim in case ... -->
```{r prepare-plot_sim}
file_path <- file.path(tmp_dir,"sim-wheat.RData")
if (!file.exists(file_path)) {
...
<div class="alert alert-info">
The data structure of the observations returned by `get_obs()` follow the same format than the simulations (*i.e.* `get_sim()` output). It is a list of `data.frame`s named after the USM name. Each row in the `data.frame` is a daily observation, and each columns is a variable.
</div>


#### Getting observation data

* Get all the observation data included in the `example` workspace (`workspace_xml`)
...
```{r prepare-plot_sim_obs2}
file_path <- file.path(tmp_dir,"sim_obs.RData")
if (!file.exists(file_path)) {
  results2 <- get_sim(workspace = workspace_xml, usm_name = c("wheat", "maize"), verbose = FALSE)
  observations2 <- get_obs(workspace = workspace_xml, usm_name = c("wheat", "maize"), verbose = FALSE)
...
```{r plot_simobs1-solution, exercise.setup = "prepare-plot_sim_obs2"}
# SOLUTION
...
```{r plot_simobs2-solution, exercise.setup = "prepare-plot_sim_obs2"}
# SOLUTION
...
```{r plot_simobs3-solution, exercise.setup = "prepare-plot_sim_obs2"}
# SOLUTION
...
```{r prepare-plot_sim_obs_4}
usms <- c("wheat","DurumWheat", "maize", "sorghum")
file_path <- file.path(tmp_dir,"sim_obs_2.RData")
if (!file.exists(file_path)) {
  results4 <- get_sim(workspace = workspace_xml, usm_name = usms, verbose = FALSE)
  observations4 <- get_obs(workspace = workspace_xml, usm_name = usms, verbose = FALSE)
...
```{r plot_simobs4-solution, exercise.setup = "prepare-plot_sim_obs_4"}
# SOLUTION
# usms <- c("wheat","DurumWheat", "maize", "sorghum")
...
```{r stat_simobs1-solution, exercise.setup = "prepare-plot_sim_obs_4"}
# SOLUTION
#usms <- c("wheat","DurumWheat", "maize", "sorghum")
...
```{r stat_simobs2-solution, exercise.setup = "prepare-plot_sim_obs_4"}
# SOLUTION
# usms <- c("wheat","DurumWheat", "maize", "sorghum")
# results <- get_sim(workspace = workspace_xml, usm_name = usms, var = c("lai_n","masec_n"))
# observations <- get_obs(workspace = workspace_xml, usm_name = usms, var = c("lai_n","masec_n"))
...
```{r stat_simobs3-solution, exercise.setup = "prepare-plot_sim_obs_4"}
# SOLUTION
# usms <- c("wheat","DurumWheat", "maize", "sorghum")
# results <- get_sim(workspace = workspace_xml, usm_name = usms, var = c("lai_n","masec_n"))
# observations <- get_obs(workspace = workspace_xml, usm_name = usms, var = c("lai_n","masec_n"))
...
```{r stat_simobs4-solution, exercise.setup = "prepare-plot_sim_obs_4"}
# SOLUTION
# usms <- c("wheat","DurumWheat", "maize", "sorghum")
# results <- get_sim(workspace = workspace_xml, usm_name = usms, var = c("lai_n","masec_n"))
# observations <- get_obs(workspace = workspace_xml, usm_name = usms, var = c("lai_n","masec_n"))
...
```{r stat_simobs5-solution, exercise.setup = "prepare-plot_sim_obs_4"}
# SOLUTION
# usms <- c("wheat","DurumWheat", "maize", "sorghum")
# results <- get_sim(workspace = workspace_xml, usm_name = usms, var = c("lai_n","masec_n"))
# observations <- get_obs(workspace = workspace_xml, usm_name = usms, var = c("lai_n","masec_n"))
...
This tutorial stores a JavaSticsinstallation in the `javastics` variable, and an example workspace in the `workspace_xml` variable.
...
```{r gen_varmod-hint-1}
# Use `workspace_path = workspace_xml`
```
...
The text files have been generated in subfolders of **`r workspace_xml`**; one folder per USM.

Now, list the created folders and files inside them:
...
#SOLUTION:
dirs <- file.path(workspace_xml, c("wheat","banana","soybean"))
...
dirs_ex <- dir.exists(file.path(workspace_xml,dirs))
if (!all(dirs_ex)) gen_usms_xml2txt(javastics, workspace_xml, usms_list = dirs[!dirs_ex], verbose = FALSE)
wrapper_options_file <- file.path(tmp_dir,"wrapper_options.RData")
if (!file.exists(wrapper_options_file)) {
  wrapper_options = stics_wrapper_options(javastics_path = javastics, data_dir = workspace_xml, parallel = TRUE, verbose = FALSE)
...
obs = get_obs(workspace = workspace_xml, usm_name = "wheat", verbose = FALSE)
obs_filt = filter_obs(obs, "lai_n", include = TRUE)
...
#SOLUTION:
optim_options = list(nb_rep = 3, maxeval=10, path_results = workspace_xml)
```
...
rm(res)
wrapper_options = stics_wrapper_options(javastics_path = javastics, data_dir = workspace_xml, parallel = TRUE, verbose = FALSE)
obs_filt = filter_obs(get_obs(workspace = workspace_xml, usm_name = "wheat", verbose = FALSE), "lai_n", include = TRUE)
...
gen_usms_xml2txt(javastics, workspace_xml, usms_list = c("demo_Wheat1","demo_BareSoil2","demo_maize3"))

# Run without succession
wrapper_options <- stics_wrapper_options(javastics_path = javastics, 
                                         data_dir = workspace_xml)
...
run_javastics(javastics_path=javastics,  workspace_path = workspace_xml, usms_list = c("intercrop_pea_barley"), verbose = FALSE)
sim <- get_sim(workspace = workspace_xml, usm_name="intercrop_pea_barley",var_list=c("lai_n","masec_n","mafruit"), verbose = FALSE)
```
...
sim <- get_sim(workspace = workspace_xml, usm_name="intercrop_pea_barley",
               ...
               usms_filepath = file.path(workspace_xml,"usms.xml"))
print(sim)
...
sim <- get_sim(workspace = workspace_xml, usm_name="intercrop_pea_barley",
               var_list=c("lai_n","masec_n","mafruit"),
               usms_filepath = file.path(workspace_xml,"usms.xml"), javastics_path=javastics)
print(sim)

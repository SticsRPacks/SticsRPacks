...
If you don't find the solution, click on the button `Hints`, it will first give you some advises and finally the expected solution of the exercise.
...
* Get the values of clay content after decarbonation (argi) for all soils defined in the `sols.xml` file of the `example` workspace of the JavaStics distribution.
...
* Get the values of the variable `lai(n)` simulated for USMs `wheat` and `maize`
...
* Get the values of the variables `lai(n)` and `masec(n)` simulated for USM `wheat` and plot their dynamics
...
```{r plot_sim1-hint-2}
# Then you can use the plot function as described in the CroPlotR documentation
```
...
```
```{r plot_sim2-hint-2}
# try using : overlap = list(list("lai(n)","masec(n)"))
```
...
results <- get_sim(workspace = workspace_xml, usm_name = "wheat",
                   var_list = c("lai(n)", "masec(n)"))
plot(results, overlap = list(list("lai(n)","masec(n)")))
```
...
* Get observations of `lai(n)` and `masec(n)` variables for USMs `wheat` and `maize`
...
```{r get_obs2-solution}
# SOLUTION
get_obs(workspace = workspace_xml, usm_name = c("wheat", "maize"), var_list = c("lai(n)","masec(n)"))
```
...
As for the first exercise in the "Plotting simulations results" section, get the values of the variables `lai(n)` and `masec(n)` simulated for USM `wheat` and plot their dynamics, but this time add the observation data to the plot. 
...
plot(results, var = c("lai(n)","masec(n)"), obs = observations)
```
<!-- with overlap
# ne fonctionne pas
...
Scatter plots are used for displaying simulated values against observed values. They can be produced using the same `plot()` function as for dynamics plots, with only changing the plot `type` argument.
...
results <- get_sim(workspace = workspace_xml, usm_name = c("wheat", "maize"))
observations <- get_obs(workspace = workspace_xml, usm_name = c("wheat", "maize"))
plot(results, var = c("lai(n)","masec(n)"), obs = observations, type = "scatter")
...
results <- get_sim(workspace = workspace_xml, usm_name = c("wheat", "maize"))
observations <- get_obs(workspace = workspace_xml, usm_name = c("wheat", "maize"))
plot(results, var = c("lai(n)","masec(n)"), obs = observations, type = "scatter", shape_sit = "symbol")

```

* Now, plot the scatter plot for the same variables `lai(n)` and `masec(n)` but for USMs "wheat","DurumWheat", "maize" and "sorghum", and with different symbols for the 2 groups c("wheat","DurumWheat") and c("maize","sorghum")
...
plot(results, var = c("lai(n)","masec(n)"), obs = observations, type = "scatter", shape_sit = "group", situation_group = groups)

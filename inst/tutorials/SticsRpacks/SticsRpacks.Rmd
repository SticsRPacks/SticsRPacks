...
This tutorial aims at introducing the use of these different packages with the STICS model. It was designed to be of increasing complexity, it is thus advised to follow it in order. The different sections are however independent and you can thus start it from any part using the menu on the left side.
...
Let's first define the setup and run the different configurations:
...
dyn_var_names <- c("lai_n", "zrac", "resmes", "azomes", "QNplante", "masec_n", "laimax", "mafruit", "drat", "Qles", "N_mineralisation", "totapN", "totir","cprecip")
...
sim_ws_ns = stics_wrapper(model_options = wrapper_options, param_values = c(codeinnact=1, codeh2oact=1), 
                          var_names = var_names)$sim_list

# Simulate with water stress only
sim_ws = stics_wrapper(model_options = wrapper_options, param_values = c(codeinnact=2), 
                       var_names = var_names)$sim_list

# Simulate with N stress only
sim_ns = stics_wrapper(model_options = wrapper_options, param_values = c(codeh2oact=2), 
                       var_names = var_names)$sim_list

# Simulate with water stress only
sim_without_stress = stics_wrapper(model_options = wrapper_options, param_values = c(codeinnact=2, codeh2oact=2), 
                                   var_names = var_names)$sim_list
...
```{r, echo=TRUE, eval=TRUE, message=FALSE}
plot(sim_ws_ns=sim_ws_ns, sim_ws=sim_ws, sim_ns=sim_ns, sim_without_stress=sim_without_stress, var = c("lai_n","zrac","resmes","azomes","QNplante","masec_n"))
```
...
The last part may not be straightforward for R newcomers ... however a function to filter dates in simulated data will come soon :-).


### Parameter optimization: A simple example

Here is an example use-case of an optimization of the `stamflax` parameter for a wheat crop using one situation (*i.e.* one USM):
...
But don't worry if you don't understand anything, you'll learn everything you need to know in this tutorial. We just want you to see that you can make a parameter optimization with less than 30 lines of code, including the plots with observations. And the good news is when you understand these lines of code, you'll be able to do almost anything with the packages, because it doesn't get any longer or more complex than this.  
...
By default, each element (i.e. data.frame per USM) of the returned object by `get_obs()` contains a `Plant` column in which are stored either `"plant_1"` or `"plant_2"` indicating respectively a main crop or an intercrop. 

In addition, if the `usms.xml` file path is given to the function, plant file names are set in the `Plant` column. If both the `usms.xml` file path and the JavaStics path are given to the function, plant names are set in the `Plant` column and an additionnal column indicates those which correspond to a `Principal` or an `Associated` crop. 
</div>
...
In fact, the plotting functions use a specfic syntax for variables names which contain braces. For example, `lai_n` must be used instead of `lai(n)`.
</div>

<!-- #### Exercises -->

* Plotting simulations dynamics of the variable `lai(n)` and `masec(n)` for `wheat`
* Independently
...
Evaluation of simulation results is performed using observation data with the help of  dynamics plotting, scatter graphs and calculating several statistics criteria. Observed data can be loaded by using the `get_obs()` function and they are given as inputs to functions which are going to be tested here after. 
...
```{r get_obs3, exercise=TRUE, exercise.setup = "prepare-get_sim"}
# Insert your R code here! 
```
...
See previous plotting exercises and add observation data in them using the appropriate input argument.
...
Scatter plots are used for displaying comparison between simulated variables against observations. They can be produced using the same `plot()` function as for dynamics plots, with only changing the plot `type`.
...
The plot function is able to adapt the used symbols when providing 
an optional specific argument indicating to use different shapes
either for `usms` or `groups of usms`.
...
# See help for understanding which input function argument must be added
```
...
For this use, one must limitate the number of variables to use and also the number of criteria to be plotted in order to ensure quality and readability.    

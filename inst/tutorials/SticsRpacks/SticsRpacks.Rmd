...
if (!local_javastics) {
  ...
  } else if (stics_version_num < 10.2) {
    javastics_dir <- "JavaSTICS-1.5.2-STICS-10.1.1"
    workspace_path <- "JavaSTICS-1.5.2-STICS-10.1.1/example"
    download_url <- 
      "https://w3.avignon.inrae.fr/forge/attachments/download/3165/"
    workspace_PW1 <- "tutorials/SticsRpacks/PW1_V10.1.0"
    java_target <- 17
    
  } else {
    javastics_dir <- "JavaSTICS-1.5.2-STICS-10.2.0"
    workspace_path <- "JavaSTICS-1.5.2-STICS-10.2.0/example"
    download_url <- 
      "https://w3.avignon.inrae.fr/forge/attachments/download/3337/"
    workspace_PW1 <- "tutorials/SticsRpacks/PW1_V10.2.0"
    java_target <- 17
  }
} else {
  ...
  stics_version_str <- attr(check, "version")
  stics_version_num <- SticsRFiles:::get_version_num(
    gsub(pattern = "(.*)\\_(.*)", x = stics_version_str, replacement = "\\1")
  )
  
  # Checking if the version is compatible with
  # the 4 Stics versions taken into account in the tuto data

  ...
  if (linux) {
    version_str <-
      suppressWarnings(
                    system(
                    paste(cmd,
                   '-version 2>&1 | head -n 1 | cut -d\\" -f 2 | cut -d\\. -f1'),
                              intern = TRUE))
...
if (!local_javastics) {
  ...
  if (lower_than_v10) { 
    ...
    download_url <- 
      "https://w3.avignon.inra.fr/forge/attachments/download/2482/"
    workspace_PW1 <- "tutorials/SticsRpacks/PW1_V9.2"

  } else if (stics_version < 10.1) {
    # same as zip archive name
    javastics_dir <- "JavaSTICS-1.5.1-STICS-10.0.0"
    workspace_path <- "JavaSTICS-1.5.1-STICS-10.0.0/example"
    download_url <- 
      "https://w3.avignon.inrae.fr/forge/attachments/download/3005/"
    workspace_PW1 <- "tutorials/SticsRpacks/PW1_V10.0"

  } else {
    javastics_dir <- "JavaSTICS-1.5.2-STICS-10.1.0"
    workspace_path <- "JavaSTICS-1.5.2-STICS-10.1.0/example"
    download_url <- 
      "https://w3.avignon.inrae.fr/forge/attachments/download/3158/"
    workspace_PW1 <- "tutorials/SticsRpacks/PW1_V10.1.0"
  }
} else {
  ...
  if (length(grep("10.0",attr(check, "version"))) > 0) {
    ...
    stics_version <- "V9.2"
    lower_than_v10 <- TRUE
  } else if (length(grep("10.1.0",attr(check, "version"))) > 0) {
    workspace_PW1 <- "tutorials/SticsRpacks/PW1_V10.1.0"
...
if (!lower_than_v10 && linux) {
  ...
  if (java_version < 11)
    stop("The default system java virtual machine is ",
...
use_default <- TRUE
work_dir <- file.path(normalizePath(dirname(tempdir()), winslash = "/"),
                      "SticsRPacks")
...
stress_ind_names <- c("turfac1moy", "turfac2moy", "swfac1moy", "swfac2moy",
                      "inn1moy", "inn2moy")

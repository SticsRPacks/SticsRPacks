...
get_stics_version <- function(version_string) {
  ...
  gsub(
    ...
    x = version_string, replacement = "\\2"
  )
}
# for simplifying use
...
if (!local_javastics) {
  ...
  download_url <- "https://w3.avignon.inrae.fr/forge/attachments/download/3712/JavaSTICS-latest.zip"
  ...
  stics_exe_path <- file.path(javastics_path, "bin", "stics_modulo")
  
  # Set the file executable just in case for other OS than windows
  if (linux) {
    system(paste("chmod +x",stics_exe_path))
  }
  
  workspace_path <- file.path(javastics_path, "example", fsep = "/")

  workspace_PW1 <- "tutorials/SticsRpacks/PW1_V11"
  ...
  stics_exe_path <- file.path(javastics_path, "bin", stics_exe_name)
  ...
  major_num <- (render_version(stics_version_num)[[1]])$major
  if (major_num < (render_version(get_version_num(9))[[1]])$major) {
    stop(
      "The JavaSTICS distribution ",
      basename(javastics_path),
      "is too old and not supported in this tutorial"
    )
  } else if (major_num < (render_version(get_version_num(10))[[1]])$major) {
    workspace_PW1 <- "tutorials/SticsRpacks/PW1_V9.2"
    java_target <- "8"
  } else if (major_num < (render_version(get_version_num(10.1))[[1]])$major) {
    workspace_PW1 <- "tutorials/SticsRpacks/PW1_V10"
    java_target <- "11"
  } else if (major_num < (render_version(get_version_num(11))[[1]])$major) {
    workspace_PW1 <- "tutorials/SticsRpacks/PW1_V10"
    java_target <- "17"
  } else if (major_num < (render_version(get_version_num(12))[[1]])$major) {
    workspace_PW1 <- "tutorials/SticsRpacks/PW1_V11"
    java_target <- "17"
  } else {
    stop(
...
You just have to type the R code in it and click on the `Run Code` green button on the right side to run it.

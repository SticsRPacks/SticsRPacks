...
stics_version <- SticsRFiles:::get_version_num(stics_version)
lower_than_v10 <- stics_version < 10
...
if (!local_javastics) {
  ...
  # Getting exe version
  check <- try(SticsOnR:::check_stics_exe(
    file.path(javastics_path, "bin",stics_exe_name), version = TRUE),
    silent = TRUE)
  
  # Stopping the tuto when the executable is not found
  if (class(check) == "try-error") {
    stop("The Stics executable (",
         stics_exe_name,
  ...
  # the 2 Stics versions taken into account in the tuto data
  # Stoppting if Stics is older than version 9.2
  if (length(grep("10.0",attr(check, "version"))) > 0) {
    workspace_PW1 <- "tutorials/SticsRpacks/PW1"
    stics_version <- "V10.0"
  } else if (length(grep("9.2",attr(check, "version"))) > 0) {
      workspace_PW1 <- "tutorials/SticsRpacks/PW1_V9.2"
      stics_version <- "V9.2"
      lower_than_v10 <- TRUE
    } else {
    stop("The JavaSTICS version is not supported with the tutorial!",
         "Please download ")
  }
...
# configure and dowload JavaStics
if(!local_javastics) {
...
* by defining this environment variable in your .Renviron file: edit this file (located in your Home directory, can be edited also using `usethis::edit_r_environ()`), and add a new line javastics_path="path_to_JavaSTICS”


Your R session must then be restarted in any case (in RStudio: menu Session-> Restart R).

If you do not set this environment variable, the tutorial will automatically download JavaStics to a temporary folder on your computer: `r javastics_path` which will be stored in the R object `javastics_path`. However, this takes more time and disk space, and antiviruses sometimes block the run of executables located in such temporary folder, which is why the use of a locally installed JavaStics distribution is recommended.

The exercises of the tutorial will use the USMs defined in the "example" workspace of JavaStics (stored in `r workspace_path`).
The path of this workspace is stored in the R object `workspace_path`.
:::

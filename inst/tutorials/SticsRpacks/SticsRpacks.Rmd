...
Generally, we make a first simulation *before* the optimization process because it changes the values in the files progressively. If we didn't make a simulation before, we have two solutions:

- make a simulation with `run_javastics` because the xml files were not modified
- re-generate the text files to use the values before optimization ans use the wrapper again

Because we were using the wrapper in this section, we will use the second solution.

> Re-generate the files for the usms "wheat", "banana" and "soybean" and make a simulation using

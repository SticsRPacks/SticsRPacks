...
get_java_version <- function(linux = TRUE) {
  if(linux){
    version <- as.numeric(system('java -version 2>&1 | head -n 1 | cut -d\\" -f 2 | cut -d\\. -f1', intern = TRUE))
    return(version)
  }
}
...
if(SticsRFiles:::get_version_num(stics_version) < 10) { 
  ...
  if (linux) {
    java_version <- get_java_version(linux = linux)
    if(java_version < 11)
      stop("The default system java virtual machine is ",system("java -version 2>&1", intern = TRUE)[1], 

...
<div class="alert alert-info">
This tutorial automatically downloaded JavaStics in a temporary folder of your computer: `r javastics_path`. The path of this folder is stored in the `javastics_path` R object.

It will be used in the following to perform the exercises. The exercises will use the USMs defined in the "example" workspace of JavaStics (stored in `r workspace_path`). The path of this workspace is stored in the `workspace_path` R object. 
...
wrapper_options = stics_wrapper_options(javastics_path = javastics_path, data_dir = workspace_PW1)

# Generate the files for the simulation:
gen_usms_xml2txt(javastics_path, workspace_PW1, usms_list = usm_name)
...
obs = get_obs(workspace = workspace_path, usm_name = usm)
obs_filt = filter_obs(obs, "lai_n", include = TRUE)

# Set-up the simulation environment:
wrapper_options = stics_wrapper_options(javastics_path = javastics_path, data_dir = workspace_path, parallel = TRUE)
optim_options = list(nb_rep = 3, path_results = workspace_path)

# Define the parameter to optimize and the boundary values:
param_info = list(stamflax = list(lb = 200, ub = 400))

# Generate the files for the simulation:
gen_usms_xml2txt(javastics_path, workspace_path, usms_list = usm)
...
# SOLUTION:
get_usms_list(usm_path = file.path(workspace_path, "usms.xml"))
```

<!-- ```{r get_usms_list1-check} -->
<!-- grade_result( -->
<!--   pass_if(~identical(.result, get_usms_list(usm_path = file.path(workspace_path, "usms.xml")))) -->
<!-- ) -->
<!-- ``` -->
...
#SOLUTION:
get_usms_list(usm_path = file.path(workspace_path, "usms.xml"), name = "cc_")
```
...
# SOLUTION:
result <- get_param_xml(file.path(workspace_path, "sols.xml"), c("HCCF", "HMINF", "epc", "DAF"), select = "sol", value = "solmais")
...
#SOLUTION:
result <- get_climate_txt(dirpath = workspace_path, filename = "Auzeville.2013")
sum(result$ttrr)
```
...
* Run all the USMs defined in the `example` workspace (path stored in `workspace_path`). The path to JavaStics is stored in the `javastics_path` R object.
...
# SOLUTION:
run_javastics(javastics_path = javastics_path,  workspace_path = workspace_path)
```
...
# SOLUTION:
run_javastics(javastics_path = javastics_path,  workspace_path = workspace_path, usms_list = c("banana","Turmeric"))
```
...
```{r prepare-get_sim}
usms <- get_usms_list(usm_path = file.path(workspace_path, "usms.xml"))
sim_done <- length(list.files(pattern = "^mod_s", workspace_path)) >=
  length(usms)

if (!sim_done) {
  print("Running simulations, please wait")
  run_javastics(javastics_path = javastics_path,
...
```{r get_sim2-solution}
# SOLUTION
get_sim(workspace = workspace_path, usm_name = c("banana","Turmeric"))
```
...
```{r get_sim4-solution}
# SOLUTION
get_sim(workspace = workspace_path, usm_name = c("wheat", "maize"),
        var_list = "lai_n")
```
...
The simulations (and observations) are stored using a `list` of `data.frame`s. Each `data.frame` is used to store the data from a USM, with each row being a value for a given day and each column a variable (*e.g.* `lai_n`, the daily `lai`). 
...
sim <- get_sim(workspace = workspace_path, usm_name = usms)
```

```{r load-sim, eval=FALSE}
usms <- c("wheat","DurumWheat", "maize", "sorghum")
sim <- get_sim(workspace = workspace_path, usm_name = usms, verbose = FALSE)
...
An other way to do so is to first transform the list of `data.frame` into a single `data.frame`, and add a new column called situation that defines the USM for each row. CroPlotR provides `bind_rows_sim` to do so. 
...
if (!file.exists(file_path)) {
  results <- get_sim(workspace = workspace_path, usm_name = "wheat",
                     var_list = c("lai_n", "masec_n"), verbose = FALSE)
...
```{r get_obs2-solution}
# SOLUTION
get_obs(workspace = workspace_path, usm_name = c("wheat", "maize"), var_list = c("lai_n","masec_n"))
```
...
if (!file.exists(file_path)) {
  results2 <- get_sim(workspace = workspace_path, usm_name = c("wheat", "maize"), verbose = FALSE)
  observations2 <- get_obs(workspace = workspace_path, usm_name = c("wheat", "maize"), verbose = FALSE)
...
* Use the values of the variables `lai_n` and `masec_n` simulated stored in the `results2` object and observed for USMs `wheat` and `maize` stored in the `observations` object and plot the corresponding scatter plot 
...
if (!file.exists(file_path)) {
  results4 <- get_sim(workspace = workspace_path, usm_name = usms, verbose = FALSE)
  observations4 <- get_obs(workspace = workspace_path, usm_name = usms, verbose = FALSE)
...
* Use the values of the variables `lai_n` and `masec_n` simulated and observed for USMs  `wheat`,`DurumWheat`, `maize`, and `sorghum` from `results4` and `observations4`.
Compute the statistical criteria using the `summary()` function
...
# SOLUTION
summ <- summary(results4, obs = observations4, stat = c("R2","nRMSE"))
...
# SOLUTION
summ <- summary(results4, obs = observations4, all_situations = FALSE, stat = c("R2","nRMSE"))
...
# SOLUTION:
get_param_xml(xml_file = file.path(workspace_path,"sols.xml"),  
...
# SOLUTION:
set_param_xml(xml_file = file.path(workspace_path,"sols.xml"),  
              param_name = "codenitrif", param_value = 1, out_path = file.path(workspace_path,"sols_new.xml"))
get_param_xml(xml_file = file.path(workspace_path,"sols_new.xml"),  
              param_name = "codenitrif")
```
...
1. JavaStics writes text files with the parameters values that only describe this USM
1. JavaStics calls the STICS model (*i.e* the STICS executable)
1. the STICS model reads the parameters values from the text files, runs a simulation, and writes the output files (*i.e.* `mod_bXXX.sti` and `mod_sXXX.sti`)
...
* Use the `gen_varmod` function to tell STICS we only want to simulate "lai_n", "masec_n" and "QNplante"
...
#SOLUTION:
gen_varmod(workspace_path, var_names = c("lai_n", "masec_n", "QNplante"))
```
...
#SOLUTION:
gen_usms_xml2txt(javastics_path, workspace_path, usms_list = c("wheat","banana","soybean"))
```
...
if (!all(dirs_ex)) gen_usms_xml2txt(javastics_path, workspace_path, usms_list = dirs[!dirs_ex], verbose = FALSE)
```
...
# Use `javastics_path = javastics_path` and `data_dir = workspace_path` 
...
#SOLUTION:
stics_wrapper_options(javastics_path = javastics_path, data_dir = workspace_path, parallel = TRUE)
```
...
if (!file.exists(wrapper_options_file)) {
  wrapper_options = stics_wrapper_options(javastics_path = javastics_path, data_dir = workspace_path, parallel = TRUE, verbose = FALSE)
...
if (!file.exists(file.path(tmp_dir,"comp_res.RData"))) {
  wrapper_options = stics_wrapper_options(javastics_path = javastics_path, 
                                          data_dir = workspace_path, parallel = TRUE, 
                                          verbose = FALSE)
  res1 = stics_wrapper(model_options = wrapper_options, sit_names = "wheat", 
                       var_names = c("lai_n", "masec_n"))
  res2 = stics_wrapper(model_options = wrapper_options, sit_names = "wheat", 
                       var_names = "lai_n", 
...
if (!file.exists(file.path(tmp_dir,"store-param-estim.RData"))) {
  wrapper_options = stics_wrapper_options(javastics_path = javastics_path, data_dir = workspace_path, parallel = TRUE, verbose = FALSE)
  obs = get_obs(workspace = workspace_path, usm_name = "wheat", verbose = FALSE)
  obs_filt = filter_obs(obs, "lai_n", include = TRUE)
  param_info = list(stamflax = list(lb = 200, ub = 400))
  optim_options = list(nb_rep = 3, maxeval = 10, xtol_rel = 1e-03, path_results = workspace_path, ranseed = 1234)
...
#SOLUTION:
obs = get_obs(workspace = workspace_path, usm_name = "wheat")
```
...
#SOLUTION:
optim_options = list(nb_rep = 3, maxeval=10, path_results = workspace_path)
```
...
can still continue to the next exercises because we cached the output for you (we stored the result of `estim_param()` in the object `optim_res`, but running it with the default value of `maxeval`).
...
if (!file.exists(file.path(tmp_dir,"prepare-param-estim-out.RData"))) {
  ...
  wrapper_options = stics_wrapper_options(javastics_path = javastics_path, 
                                          data_dir = workspace_path, parallel = TRUE, 
                                          verbose = FALSE)
  obs_filt = filter_obs(get_obs(workspace = workspace_path, usm_name = "wheat", 
  ...
  sim_before_optim = stics_wrapper(model_options = wrapper_options, sit_names = "wheat", 
                                   verbose = FALSE)$sim_list
...
gen_usms_xml2txt(javastics_path, workspace_path, usms_list = c("demo_Wheat1","demo_BareSoil2","demo_maize3"))

# Run without succession
wrapper_options <- stics_wrapper_options(javastics_path = javastics_path, 
                                         data_dir = workspace_path)
without_succ <- stics_wrapper(model_options = wrapper_options, 
                              sit_names = c("demo_Wheat1","demo_BareSoil2","demo_maize3"))
...
#SOLUTION:
plot(without_succ$sim_list, var = "azomes", 
     successive = list(c("demo_Wheat1","demo_BareSoil2","demo_maize3")))

plot(with_succ$sim_list, var = "azomes", 
     successive = list(c("demo_Wheat1","demo_BareSoil2","demo_maize3")))
```
...
# SOLUTION:
run_javastics(javastics_path=javastics_path,  workspace_path = workspace_path, usms_list = c("intercrop_pea_barley"))
sim <- get_sim(workspace = workspace_path, usm_name="intercrop_pea_barley",var_list=c("lai_n","masec_n","mafruit"))
```
...
if (!file.exists(file.path(workspace_path,c("mod_spintercrop_pea_barley.sti",
...
if (!file.exists(file.path(tmp_dir,"prepare-intercrop-2.RData"))) {
  sim <- get_sim(workspace = workspace_path, 
                 usm_name="intercrop_pea_barley",
                 var_list=c("lai_n","masec_n","mafruit"), verbose = FALSE)
...
# SOLUTION:
sim <- get_sim(workspace = workspace_path, usm_name="intercrop_pea_barley",
               var_list=c("lai_n","masec_n","mafruit"),
               usms_filepath = file.path(workspace_path,"usms.xml"))
print(sim)
...
sim <- get_sim(workspace = workspace_path, usm_name="intercrop_pea_barley",
               var_list=c("lai_n","masec_n","mafruit"),
               usms_filepath = file.path(workspace_path,"usms.xml"), javastics_path=javastics_path)
print(sim)

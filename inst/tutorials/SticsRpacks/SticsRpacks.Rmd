...
# configure and download JavaStics
if(!local_javastics) {
...
**WARNING: The `javastics_path` and `workspace_path` values printed above may be incorrect if you have redefined the `javastics_path` environment variable and restarted the tutorial by clicking on the "Start Tutorial" button in RStudio, or by running `learnr::run_tutorial("SticsRpacks", package="SticsRPacks")`.
To have correct values of these variables in this case, the tutorial must be started by running `learnr::run_tutorial("SticsRpacks", package="SticsRPacks", clean=TRUE)`.**

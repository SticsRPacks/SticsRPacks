...
get_java_version <- function(linux = TRUE, cmd = "java") {
  version <- NULL
  if(linux) {
    version_str <-
      suppressWarnings(system(paste(cmd, '-version 2>&1 | head -n 1 | cut -d\\" -f 2 | cut -d\\. -f1'),
             intern = TRUE))
...
if(SticsRFiles:::get_version_num(stics_version) < 10) { 
  javastics_dir <- "JavaSTICS-1.41-stics-9.2"
  workspace_path <- "JavaSTICS-1.41-stics-9.2/example"
  # to set before if/else bloc when the V10.0.0 will be out
  user_passwd <- SticsRPacks:::get_forge_userpass("public")
  download_url <- "https://w3.avignon.inra.fr/forge/attachments/download/2482/"
  workspace_PW1 <- "tutorials/SticsRpacks/PW1_V9.2"
  
  
} else { 
  # same as zip archive name
  javastics_dir <- "JavaSTICS-1.5.1-STICS-10.0.0"
  workspace_path <- "JavaSTICS-1.5.1-STICS-10.0.0/example"
  # to remove when the V10.0.0 will be out
  user_passwd <- SticsRPacks:::get_forge_userpass("public")
  download_url <- "https://w3.avignon.inrae.fr/forge/attachments/download/3005/"
  workspace_PW1 = "tutorials/SticsRpacks/PW1"
  #java <- Sys.getenv("JAVA")
  ...
  if (linux) {
    ...
    java_version <- get_java_version(linux = linux, cmd = java)
    ...
    if(java_version < 11)
      stop("The default system java virtual machine is ",system(paste(java,"-version 2>&1"), intern = TRUE)[1], 
           #"\nThis is not correct for running JavaStics\n", 
           "\nA java 11 version installation is needed for running JavaSTICS, \n",
...
if(SticsRFiles:::get_version_num(stics_version) < 10) { 
  ...
  out_path <- javastics_path
} else {
  out_path <- work_dir
}

files_list <- list.files(javastics_path)
if(length(files_list) == 0 || !any(grepl("JavaStics.exe",files_list))) {
  zip_path <- file.path(work_dir,paste0(javastics_dir,".zip"))
  system(paste0("curl -u ",user_passwd," ", download_url, javastics_dir,".zip --output ", zip_path))
...
# Generate the files for the simulation:
gen_usms_xml2txt(javastics_path, workspace_PW1, usm = usm_name, java_cmd = knitr::opts_chunk$get("java_cmd"))

```

```{r PW1-setup, echo=FALSE, eval=!knitr::opts_chunk$get("linux"), message=FALSE}
...
p <- ggplot(df_long, aes(fill=Case, y=value, x=variables)) + geom_bar(position="dodge", stat="identity")
print(p)
```
...
#SOLUTION:
p <- plot("Without successive"=without_succ$sim_list,
